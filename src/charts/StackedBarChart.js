...
function StackedChart({ data,keys, height, scaleX, scaleY, setTooltip,margin,colors,barColor }) {
  const ref = useRef();
  ...
  useEffect(() => {
    ...
    select(ref.current)
      ...
      .data(layer => layer)
      .join("rect")
      .attr("x", sequence => scaleX(sequence.data.label))
      .attr("width", scaleX.bandwidth())
      .attr("y", sequence => scaleY(sequence[1]))
      .attr("height", sequence => scaleY(sequence[0]) - scaleY(sequence[1]));
      // .attr("height", 10);
    }
  }, [scaleX,scaleY,colors, data, keys]);
...
export default function StackedBarChart({w,data,keys,colors,barColor,minValue }) {
  let h = w/1.5;
  ...
  const scaleY = scaleLinear()
    // .domain([0, Math.max(...data.map(({ value }) => value.reduce((a,b)=>a+b,0)))*1.12])
    .domain([0, 25])
  ...
  return (
    ...
    >
      <g transform={`translate(${margin.left}, ${margin.top})`}>
          ...
          <StackedChart data={data} keys={keys} height={height} scaleX={scaleX} scaleY={scaleY} setTooltip={setTooltip} margin={margin} colors={colors} barColor={barColor} />
      </g>
    </svg>
      {tooltip !== null ? (
